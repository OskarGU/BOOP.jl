...
# Add sampled points.
scatter!(p2, rescale.(X_final[:,1], x_lo[1], x_hi[1]), rescale.(X_final[:,2], x_lo[2], x_hi[2]), label="Sampel", mc=:red, ms=4, legend=false);
plot(p1,p2, size=(1400,400));

# Slice plots at specific discrete z-values
z_slices = round.([rescale(5,x_lo[2], x_hi[2]), rescale(7, x_lo[2], x_hi[2]), rescale(8,x_lo[2], x_hi[2])], digits=2) # 7 is optimum
slice_plots = []

function findClosest(vec::Vector{T}, target::T) where T <: Number
...
for z_val in z_slices
    ...
    pts = hcat(collect(xs1), fill(float(z_val), length(xs1)))'
    ...
    p = plot(xs1, Z_true[:,findClosest(collect(xs2), inv_rescale(z_val, x_lo[2], x_hi[2]))], label="Sanning (z=$z_val)", lw=2, lc=:black, title="Snitt vid z = $z_val", ylims=(-10, 8))
    plot!(p, xs1, y_slice_model, label="GP Mean", lw=2, lc=:blue, ribbon=(1.96*σ_slice_model, 1.96*σ_slice_model), fillalpha=0.2, fc=:blue)
...
z_plot_grid = rescale.(range(0, 13, length=100), x_lo[2], x_hi[2])
